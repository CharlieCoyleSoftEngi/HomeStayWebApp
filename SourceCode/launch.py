...
def  HomePageSlected(name=None):
		try:
			...
			UserType = "Visitor"
		db= get_db()
		data = db.cursor().execute("SELECT * FROM Vacancies WHERE available = 1")
...
def  ProfileSlected(name=None):
	try:
			...
			return render_template("host.html", userContent=userContent, userListings=userListings,  userApplications=userApplications,userAcceptedApplications=userAcceptedApplications , negStar1=negStar1, posStar1=posStar1, urlHelper=urlHelper )
		elif session['Type'] == 'Applicant':
			db = get_db()
...
@app.route('/homestay/profile/student/<id>', methods=["POST","GET"])
def  StudentSlected(name=None, id=None):
                db = get_db()
                Host = query_db('SELECT * FROM Applicants WHERE applicantID = ?', [id], one=True)
                negStar = 10 - int(Host[7])
                negStar1 = range(negStar)
                posStar1 = range(int(Host[7]))
                userListings = db.cursor().execute('SELECT * FROM Vacancies WHERE hostID = ?', [Host[0]])
                userListings = list(userListings.fetchall())
                urlHelper = "../../../homestay/listing/"
                return render_template("student.html", userContent=Host, userListings=userListings, negStar1=negStar1, posStar1=posStar1, urlHelper=urlHelper )
...
def  viewApllication(name=None, Vid=None, Aid=None):
	...
	if request.method == 'POST':
		Aid = Aid[:-1]
		db.cursor().execute('UPDATE Applications SET accepted = 1 WHERE applicantID = ? AND vacancyID = ?',(Aid, Vid))
		db.commit()
		db.cursor().execute('UPDATE Vacancies SET available = 0 WHERE vacancyID = ?',(Vid))
		db.commit()
		return redirect(url_for(".HomePageSlected"))
	if session['Type']=='Host' or session['Type']=='Applicant':
		...
		location = str(location[0])
		ButtonLink = "../../../../../homestay/profile/student/"+ str(Aid)
		return render_template("viewapplication.html", Location=location, hEmail=hEmail, aEmail=aEmail, Application=application[3],HostCheck=HostCheck ,ButtonLink=ButtonLink )
	else:
		return render_template("listingtemp.html")
